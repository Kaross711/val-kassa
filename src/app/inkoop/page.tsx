...
// Extra types om 'any' te vermijden bij Supabase responses
type OrderItemRow = {
    quantity: number;
    units_per_box: number;
    actual_quantity: number;
...
- Als geen totaalbedrag zichtbaar is, zet total_amount op 0`,
            ...
            if (err instanceof Error) {
                ...
                setError("Fout bij verwerken foto");
            }
        } finally {
            setProcessing(false);
        }
    ...
    async function savePurchaseOrder() {
        ...
        try {
            ...
            const orderItems = items.map((item) => ({
                ...
                units_per_box: item.units_per_box,
                actual_quantity: item.actual_quantity,
                unit_price: 0,   // <- belangrijk ivm NOT NULL constraint
                line_total: 0,   // optioneel: hier ook 0 bewaren
            }));
