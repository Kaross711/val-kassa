...
export default function KassaPage() {
    ...
    async function loadProducts() {
        ...
        // Voorraad ophalen voor alle producten
        ...
        if (ids.length > 0) {
            ...
            // Voeg voorraad informatie toe aan alle producten
    ...
    function addProduct(p: Product) {
        ...
        // Waarschuwing bij geen voorraad, maar sta het wel toe
        if (p.stock_quantity === null || p.stock_quantity <= 0) {
            setNotification(`Let op: "${p.name}" is niet op voorraad!`);
        }
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8 gap-2">
                    {filtered.map((p) => {
                        ...
                        return (
                            ...
                            >
                                {outOfStock && (
                                    <div className="absolute top-1 right-1 text-[9px] px-1.5 py-0.5 rounded-full bg-red-500 text-white font-semibold leading-none">
                                        Niet op voorraad
                                    </div>
                                )}
                                <div className="flex items-start justify-between gap-1 mb-1">
                                    <div className={`font-medium text-xs leading-tight break-words flex-1 ${
                                        outOfStock ? "text-orange-900 pr-16" : "text-slate-900"
                                    }`}>
                                        {p.name}
                                    </div>
                                    {p.unit === "KILO" && (
